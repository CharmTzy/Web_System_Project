...
try {
    $chatData = $chatService->overview($userId, 'admin', $requestedConversationId);
} catch (Throwable $exception) {
    report_exception($exception, 'admin.chat');
    $chatError = safe_exception_message($exception, 'We could not load the chat dashboard right now.');
    $chatData = [
        'conversations' => [],
...
<main>
    <section class="hero-section hero-section--compact">
        <div class="container">
            <span class="hero-section__eyebrow">Admin chat</span>
            <h1 class="hero-section__title" style="max-width:18ch;">Conversation Monitor</h1>
            <p class="hero-section__copy">Watch customer and seller discussions in real time and step in whenever support is needed.</p>
        </div>
    </section>
