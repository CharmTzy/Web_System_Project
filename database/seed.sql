...
INSERT INTO help_categories (id, name, slug, description, icon_key, sort_order) VALUES
    ...
    (3, 'Payments', 'payments', 'Cards, wallet payments, billing checks, and failed checkouts.', 'wallet', 3),
...
INSERT INTO help_questions (id, category_id, question, answer, is_hot, sort_order) VALUES
    ...
    (4, 4, 'How do I track my order after payment?', 'Go to your profile or orders page, open the order, and review the shipping status shown by the seller or courier. Tracking updates usually appear shortly after the order is packed and handed over for delivery.', 1, 4),
    ...
    (6, 3, 'Why did my payment fail at checkout?', 'Payment can fail if your card details are incorrect, the bank blocks the transaction, the payment session expires, or the selected method has insufficient balance. Recheck the details or try another payment method.', 1, 6),
    ...
    (8, 2, 'How do I use a promo code or reward voucher?', 'Apply the promo code during checkout in the voucher section before placing the order. Some rewards require a minimum spend, a valid campaign period, or specific product eligibility, so check the promotion terms if the discount is not applied.', 1, 8);
